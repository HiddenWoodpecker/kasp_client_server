...
# Замечания
 * Для имитации сложных вычислений над файлом  после поиска каждого паттерна  процесс засыпает на 2 секунды(Даже для маленького файла это в сумме будет 8 секунд для конфига из [example]) (засыпает через usleep(2000000))
